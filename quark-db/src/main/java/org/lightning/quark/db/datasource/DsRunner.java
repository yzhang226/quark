 ...
 */
public class DsRunner extends AbstractQueryRunner {

    private static final Logger logger = LoggerFactory.getLogger(DsRunner.class);
     ...
     */
    public DsRunner(boolean pmdKnownBroken) {
     ...
     */
    public DsRunner(StatementConfiguration stmtConfig) {
        super(stmtConfig);
    }
     ...
     * and if it breaks, we'll remember not to use it again.
     */
    public DsRunner(DataSource ds, boolean pmdKnownBroken) {
     ...
     * @param stmtConfig The configuration to apply to statements when they are prepared.
     */
    public DsRunner(DataSource ds, StatementConfiguration stmtConfig) {
        super(ds, stmtConfig);
    }
     ...
     * @param stmtConfig The configuration to apply to statements when they are prepared.
     */
    public DsRunner(DataSource ds, boolean pmdKnownBroken, StatementConfiguration stmtConfig) {
    ...
    private <T> T insertBatch(Connection conn, boolean closeConn, String sql, ResultSetHandler<T> rsh, Object[][] params)
        ...
        try {
            ...
            try {
                ...
                logger.error("getGeneratedKeys error");
            }
        ...
        if (params != null) {
            for (int i = 0; i < params.length; i++) {
                if (params[i] instanceof OutParameter) {
                    throw new QuarkExecuteException("unsupported out-parameter");
